...
- [ ] **(M) (refactor) Give GltfLoader's four texture slots one table instead of three copies of the `has_pbr_metallic_roughness` guard** — base-colour, metallic-roughness, normal and emissive each need a UV transform, a TEXCOORD warning and an sRGB-keyed slot assignment, and those three concerns are currently spelled out slot-by-slot in two functions with the pbr guard written three times.

  **Files to read:**
  - `Src/GraphicsEngineVulkan/scene/GltfLoader.cpp` — `warnUnsupportedTexCoordSet`,
    `readUvTransform` / `UvTransformRows`, `fromGltfMaterial` (the four
    `readUvTransform` calls split across one `has_pbr_metallic_roughness`
    guard, then the four `warnUnsupportedTexCoordSet` calls split across a
    *different* one, in a different order), and `parseCpu`'s
    `assignTextureSlot` loop (a third copy of the same guard, choosing the
    srgb flag per slot).
  - `Src/shared/scene/ObjMaterial.hpp` — the per-slot row pairs and the
    `*TextureID` sentinels the table must fill.
  - `Test/commit/VulkanEngine/gltfParseSuite.cpp` — `KhrTextureTransformIsReadPerTextureSlot`,
    `ATransformOnANonBaseSlotAloneStillReachesTheMaterial`,
    `MetallicRoughnessTextureGetsItsOwnLinearSlot`,
    `NormalAndBaseColourSharingOneImageGetTwoSlotsForDifferentColourSpaces` —
    the existing per-slot coverage this refactor must keep green, and the
    fixture-building pattern to copy for the new test.
  - `Test/commit/VulkanEngine/buildIntegritySuite.cpp` — `NoShaderRedeclaresTheCascadeCount`,
    the source-scanning gate whose structure the new gate copies (including
    anchoring exemptions to a source marker, not a line number).

  **Steps:**
  1. In `GltfLoader.cpp`'s anonymous namespace, add one description of a
     texture slot — e.g. `struct GltfTextureSlot { const cgltf_texture_view *view;
     const char *label; bool srgb; }` — and a single function
     `gltfTextureSlots(const cgltf_material &)` returning the four in a fixed
     order (base-colour, metallic-roughness, normal, emissive) with their
     labels and colour spaces. The two pbr slots must still be gated on
     `has_pbr_metallic_roughness`: keep the guard *inside* this one function
     (point those two entries at a static all-zero `cgltf_texture_view` when
     the block is absent, so downstream `view.texture == nullptr` checks
     behave exactly as today). This is the only place the guard survives.
  2. Rewrite `fromGltfMaterial` to loop over `gltfTextureSlots(material)`
     once, calling `warnUnsupportedTexCoordSet` and `readUvTransform` per
     entry and writing the rows into the matching `ObjMaterial` member pair.
     Warning **order changes** as a result (it becomes the table order); that
     is fine, nothing asserts on ordering, but say so in the commit message.
  3. Rewrite `parseCpu`'s per-material block to loop over the same table,
     assigning `objMaterial.<slot>TextureID = assignTextureSlot(view, slot.srgb)`.
     Keep `assignTextureSlot` exactly as it is — the (image, sampler, srgb)
     dedup is not what this task changes.
  4. Keep every existing comment's *content* (the sRGB-vs-linear rationale,
     the TEXCOORD_0-only limitation, the per-slot KHR_texture_transform
     reasoning) but attach it to the table entry it explains instead of
     repeating it per call site.
  5. Confirm the change is behaviour-neutral by running the whole
     `GltfParseUnit.*` suite, not just the new test.

  **Test:** Two additions to `Test/commit/VulkanEngine/gltfParseSuite.cpp`
  (pure CPU, no device — build a `GltfLoader{}` and `parseCpu` a written-out
  fixture, exactly like `KhrTextureTransformIsReadPerTextureSlot` does):
  - `GltfParseUnit.AllFourTextureSlotsRoundTripTogether` — one material
    carrying all four textures, each with a *distinct* KHR_texture_transform
    and each pointing at a distinct image; assert all four `*TextureID`s are
    >= 0 and mutually distinct, all four row pairs match their own transform,
    and `getTextureSrgbFlags()` is 1 for the base-colour/emissive slots and 0
    for the normal/metallic-roughness slots. No current test exercises all
    four at once, which is exactly what a table rewrite can silently
    cross-wire.
  - `BuildIntegrity.GltfTextureSlotsAreEnumeratedInOneTable` — scan
    `Src/GraphicsEngineVulkan/scene/GltfLoader.cpp` and fail if
    `has_pbr_metallic_roughness` appears more than once outside
    `gltfTextureSlots`, or if `readUvTransform(` / `warnUnsupportedTexCoordSet(`
    / `assignTextureSlot(` appear at more than one call site each. Follow
    `NoShaderRedeclaresTheCascadeCount`'s structure and anchor any exemption
    to a source marker.

  **Build:** `clangcl-debug`. Same container invocation as the task above, then
  from the repo root:
  `.\build-clangcl-debug\commitTestSuite.exe --gtest_filter=GltfParseUnit.*:BuildIntegrity.GltfTextureSlots*`

  **Context:** Same shape as `f97712f4` (one `ShaderStagePair` instead of six
  copies) and `a3b42dfc` (one depth-aspect definition): the payoff is that the
  *fifth* slot — occlusion, the one material feature the Rust/WGSL forward path
  has and the C++ paths do not (see `docs/shader-sharing.md`'s material-feature
  matrix) — becomes one table row plus its shader work, instead of six more
  hand-copied call sites. No GPU verification is needed: nothing here touches
  the render path, and the loader suites run headless. Do NOT extend this task
  into adding the occlusion slot — land the table first.

- [ ] **(S) (refactor) Share the two `uploadParsed` preconditions and cover the error paths neither loader tests** — `ObjLoader::uploadParsed` and `GltfLoader::uploadParsed` each hand-roll the same device-free / empty-parse guard pair with differently-worded messages, and both early returns are completely untested despite being pure-CPU reachable.

  **Files to read:**
  - `Src/GraphicsEngineVulkan/scene/ModelAssembly.ixx` — `addTextureOrDefault`,
    `ensureAtLeastOneTexture`, `addMeshesForRanges`. This module's leading
    comment already declares itself "the shared tail of ObjLoader::uploadParsed
    and GltfLoader::uploadParsed"; the guards are the part that never moved.
  - `Src/GraphicsEngineVulkan/scene/ObjLoader.cpp` — `uploadParsed`'s guards
    (`"uploadParsed called on a device-free ObjLoader"` /
    `"uploadParsed called before a successful parseCpu"`).
  - `Src/GraphicsEngineVulkan/scene/GltfLoader.cpp` — `uploadParsed`'s guards,
    worded differently (`"GltfLoader::uploadParsed called on a device-free loader"`).
  - `Src/GraphicsEngineVulkan/scene/{ObjLoader,GltfLoader}.ixx` — both declare a
    default (device-free) constructor, which is what makes these paths testable
    with no Vulkan at all.
  - `Test/commit/VulkanEngine/objParseSuite.cpp` — `ObjParseUnit.ParsesWithoutAVulkanDevice`
    is the device-free harness pattern to copy.
  - `Test/commit/VulkanEngine/buildIntegritySuite.cpp` —
    `BuildIntegrity.ModelUploadGoesThroughTheSharedAssembly`, the existing gate
    that pins the shared-assembly rule; extend its banned-call list rather than
    writing a second gate.

  **Steps:**
  1. Add to `ModelAssembly.ixx`:
     `bool uploadPreconditionsMet(const std::shared_ptr<VulkanDevice> &device,
     std::size_t vertexCount, const char *loaderName)` — logs
     `spdlog::error("{}::uploadParsed called on a device-free loader", loaderName)`
     or `"... called before a successful parseCpu"` and returns false, true
     otherwise. One wording for both loaders; `loaderName` is what differs.
  2. Replace both guard pairs with
     `if (!uploadPreconditionsMet(device, vertices.size(), "ObjLoader")) { return nullptr; }`
     (and `"GltfLoader"`). Check order must stay device-first, then
     empty-parse — a device-free loader that also never parsed must keep
     reporting the device, which is the more actionable error.
  3. Add `"uploadPreconditionsMet("` to
     `ModelUploadGoesThroughTheSharedAssembly`'s `kBannedCalls` with
     `ModelAssembly.ixx` exempt, so a third loader cannot hand-roll the guards
     back in. Update that test's leading comment to name the guards as the
     third thing the shared assembly now owns.
  4. Leave `ObjLoader.ixx` / `GltfLoader.ixx`'s doc comments on `uploadParsed`
     as they are — they already describe this contract correctly.

  **Test:** Four new pure-CPU tests, no device and no GPU:
  - `ObjParseUnit.UploadParsedOnADeviceFreeLoaderReturnsNull` and
    `GltfParseUnit.UploadParsedOnADeviceFreeLoaderReturnsNull` — default-construct
    the loader, `parseCpu` a real fixture (assert it succeeded), then
    `EXPECT_EQ(loader.uploadParsed(), nullptr)`.
  - `ObjParseUnit.UploadParsedBeforeAnyParseReturnsNull` and
    `GltfParseUnit.UploadParsedBeforeAnyParseReturnsNull` — default-construct
    and call `uploadParsed()` with no parse at all; also `nullptr`.
  Use `sceneConfig::resolveModelPath` + the existing `GTEST_SKIP()`-if-missing
  guard the suites already use for fixtures.

  **Build:** `clangcl-debug`. Same container invocation as above, then from the
  repo root:
  `.\build-clangcl-debug\commitTestSuite.exe --gtest_filter=ObjParseUnit.UploadParsed*:GltfParseUnit.UploadParsed*:BuildIntegrity.ModelUploadGoesThroughTheSharedAssembly`

  **Context:** The refactor is small; the coverage is the point. Both early
  returns are the "programming error, not a runtime condition" path the header
  comments promise logs-not-silence for, and nothing has ever executed them —
  `AsyncModelParse` hands parsed state to a device-owning loader, so the guards
  only fire when that wiring breaks. Follows the same pattern as `994cbf4a`
  (collapse duplicated teardown across the two rasterizers): consolidate, then
  pin with a gate so it cannot re-duplicate.
