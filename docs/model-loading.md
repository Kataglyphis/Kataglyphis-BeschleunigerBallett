...
| `metallicRoughnessTextureID` | dedup'd slot into `textureImages` (same `imageSlot` map as `textureID`/`emissiveTextureID`/`normalTextureID`), uploaded linear; `-1` if the material has no `pbrMetallicRoughness` block or no `metallicRoughnessTexture` | — | `material_fetch.slang`'s `material_metallic_roughness()` in the three BRDF paths; `path_tracing.slang`, metallic term only |
