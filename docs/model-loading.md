...
| `shininess` | — (only ever set to a derived `mix(128, 1, roughnessFactor)` value, kept as the OBJ-only fallback `material_roughness()` falls back to) | `Ns` | `material_rules.slang`'s `material_roughness()`, OBJ materials only |
