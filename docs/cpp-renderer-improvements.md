...
- **VMA adoption** — allocator exists (with `BUFFER_DEVICE_ADDRESS`) but every
  buffer/image still gets a dedicated raw `vkAllocateMemory`. Plan: Allocator
  moves into `VulkanDevice`; `VulkanBuffer`/`VulkanImage` convert to
  `vmaCreateBuffer/Image` with API-compatible signatures; persistently-mapped
  UBOs via `VMA_ALLOCATION_CREATE_MAPPED_BIT`; RT device-address buffers kept
  working (render-modes GPU test gates the change).

## Queued (design notes)

1. **Persistent staging ring** — `CommandBufferManager::endAndSubmitCommandBuffer`
   blocks on `queue.waitIdle()` per upload and staging buffers are
   created/destroyed per call. Replace with a persistent ring + fence; removes
   hitching on model load/transform/reload.
2. **Pipeline cache + prebuilt SPIR-V** — no `VkPipelineCache` anywhere; GLSL
